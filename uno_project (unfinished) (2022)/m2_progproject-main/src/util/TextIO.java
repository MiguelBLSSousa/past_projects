...
/**
 * util.TextIO provides a set of static methods for reading and writing text.  By default, it reads
 ...
 * behavior; to handle this case, util.TextIO will give up after 10 consecutive illegal inputs and
 ...
 *
 * <p>This class requires Java 5.0 or higher. (A previous version of util.TextIO required only Java 1.1;
...
public class TextIO {
    ...
    /* Modified November 2007 to empty the util.TextIO input buffer when switching from one
    ...
    /**
     * If util.TextIO is currently reading from a file, then the return value is the name of the file.
    ...
    /**
     * If util.TextIO is currently writing to a file, then the return value is the name of the file.
    ...
    public static void putf(String format, Object... items) {
        if (format == null) {
            throw new IllegalArgumentException("Null format string in util.TextIO.putf() method.");
        }
        try {
            out.printf(format, items);
        } catch (IllegalFormatException e) {
            throw new IllegalArgumentException("Illegal format string in util.TextIO.putf() method.");
        }
        out.flush();
        if (out.checkError()) {
            outputError("Error while writing output.");
     ...
     * end-of-file character (specified by the constant util.TextIO.EOF).An end-of-line is always returned
    ...
    private static void outputError(String message) {  // Report an error on output.
        if (writingStandardOutput) {
            System.err.println("Error occurred in util.TextIO while writing to standard output!!");
            outputErrorCount++;
