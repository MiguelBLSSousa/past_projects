...
function hideBookingModal(e) {
    if (e.target === document.getElementById("bookingModal") || e.target === document.getElementById("close-booking-modal-btn"))
        document.getElementById("bookingModal").style.display = "none";

}

function handleBookingModal(day, time, date) {

    const bookingModal = document.getElementById("bookingModal");

    //set the display in style to null
    bookingModal.style.display = null;

    //get dateTimeContainer set text date, time
    const dateTimeContainer = document.getElementById("dateTimeContainer")
    dateTimeContainer.innerText = date + ": " + time;

    const bookBtn = document.getElementById("book-lesson-btn");
    //remove listeners
    const bookBtnClone = bookBtn.cloneNode(true);
    bookBtn.parentElement.replaceChild(bookBtnClone, bookBtn);
    bookBtnClone.addEventListener('click', async function() {
        const bookingSuccessful = await bookLesson(day, scheduleWeek, scheduleYear, time, parseInt(time.slice(0, 2)) + 1 + time.slice(2));
...
function onSlot(element) {
    ...
    if (element.classList.contains("active")) {
        handleBookingModal(day, time, date)
    }
}
...
async function bookLesson(day, week, year, startTime, endTime) {
    let searchParams = new URLSearchParams(location.search);
    let tid = searchParams.get("tid");
    ...
    try {
        const response = await fetch(`./api/teachers/${tid}/book`, {
            method: 'PUT',
            headers: {
...
async function sendNotification() {
    ...
    fetch(`api/teachers/${tid}/teacher_notification`, {
        method: 'PUT',
        headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
